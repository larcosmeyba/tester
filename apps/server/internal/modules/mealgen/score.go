...
func Score(recipe meals.Recipe, request meals.PlanRequest, catalog *meals.Catalog, pantry map[string]bool) float64 {
	var total float64
	...
	liked := meals.Set(request.Likes.Ingredients...)
	...
	if request.CookingTime.MaxMinutes != nil && request.CookingTime.Strength == meals.StrengthPreferred &&
...
func pantryScore(recipe meals.Recipe, request meals.PlanRequest, catalog *meals.Catalog, pantry map[string]bool) float64 {
	lines := meals.PurchasableLines(recipe)
...
func matchesStyle(recipe meals.Recipe, style string) bool {
	switch style {
	case "quick_easy":
