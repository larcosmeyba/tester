...
func TestNewAnthropicRequiresKey(t *testing.T) {
	if _, err := New(Config{Kind: "anthropic"}); err == nil {
		t.Error("expected an error when MEAL_AI_API_KEY is missing, got nil")
	}
}
...
func TestNewProviderSelection(t *testing.T) {
	...
	if _, err := New(Config{Kind: "bogus"}); err == nil {
		t.Error("expected an error for an unknown MEAL_AI_PROVIDER, got nil")
	}
}
...
func TestAnthropicCompleteRequestShape(t *testing.T) {
	resp, header, body := anthropicRoundTrip(t,
		Request{System: "Be brief.", User: "Plan my week.", MaxTokens: 300},
		okAnthropicBody("Here is your week."))
	...
	if body["max_tokens"] != float64(300) {
		t.Errorf("body max_tokens = %v, want 300", body["max_tokens"])
	}
	...
	if !ok || msg["role"] != "user" || msg["content"] != "Plan my week." {
		t.Errorf("body messages[0] = %v, want role=user with the user prompt", messages[0])
	}

	if resp.Text != "Here is your week." {
...
func TestAnthropicCompleteOmitsEmptySystem(t *testing.T) {
	_, _, body := anthropicRoundTrip(t,
		Request{User: "Plan my week.", MaxTokens: 300},
