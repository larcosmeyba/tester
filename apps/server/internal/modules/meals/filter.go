...
func PurchasableLines(recipe db.Recipe) []db.RecipeIngredient {
	var lines []db.RecipeIngredient
...
// assumed safe.
func ViolatesAllergy(recipe db.Recipe, request PlanRequest, catalog *Catalog) bool {
	if len(request.Allergies) == 0 && len(request.AllergyIngredients) == 0 {
		return false
	}
	named := set(request.AllergyIngredients...)
	...
	for _, line := range recipe.Ingredients {
		...
		for _, allergy := range request.Allergies {
			if ContainsAllergen(ingredient, allergy.Allergen) {
				return true
			}
		}
	}
...
func ViolatesDiet(recipe db.Recipe, request PlanRequest, catalog *Catalog) bool {
	var required []string
	for _, requirement := range request.DietaryRequirements {
		if requirement.Strength == StrengthRequired {
	...
	for _, line := range recipe.Ingredients {
		...
		for _, diet := range required {
			if !SatisfiesDiet(ingredient, diet) {
				return true
			}
		}
	}
...
func ViolatesDislikes(recipe db.Recipe, request PlanRequest, catalog *Catalog) bool {
	if len(request.Dislikes.Ingredients) == 0 {
		return false
	}
	disliked := set(request.Dislikes.Ingredients...)
	for _, line := range recipe.Ingredients {
		if line.IngredientID == nil {
			continue
		}
...
func ViolatesEquipment(recipe db.Recipe, request PlanRequest) bool {
	available := set(request.Equipment...)
...
func ViolatesTime(recipe db.Recipe, request PlanRequest) bool {
	if request.CookingTime.MaxMinutes == nil || request.CookingTime.Strength != StrengthRequired {
		return false
	}
...
func EligibleRecipes(recipes []db.Recipe, request PlanRequest, catalog *Catalog) []db.Recipe {
	excluded := set(request.ExcludeRecipeIDs...)
	var eligible []db.Recipe
