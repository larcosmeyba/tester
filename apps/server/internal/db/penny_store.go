...
// PennyProposedAction finds a pending action by its id.
//
// The action is read back from the server's own record rather than accepted
// from the client on confirmation. That is the point of storing it: what runs
// when a user taps "yes" is what Penny proposed and what the confirmation card
// described, not whatever a modified app sends back with the tap.
func (s *Store) PennyProposedAction(ctx context.Context, userID, actionID string) (penny.ProposedAction, string, error) {
	const query = `
		SELECT proposed_action, conversation_id
		FROM penny_messages
		WHERE user_id = $1
		  AND proposed_action IS NOT NULL
		  AND proposed_action->>'id' = $2
		LIMIT 1`
	...
	if err := s.pool.QueryRow(ctx, query, userID, actionID).Scan(&encoded, &conversationID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return penny.ProposedAction{}, "", ErrProposedActionNotFound
		}
		return penny.ProposedAction{}, "", err
	}
...
var ErrProposedActionNotFound = errors.New("proposed action not found")
